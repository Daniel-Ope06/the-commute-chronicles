...
```{r bus-trip-origin, echo=FALSE, fig.show="hold", fig.align='center', out.width="90%", fig.cap="Data Collection Source. The composite image shows the physical bus stop (left), the Google Maps route of the journey (center), and the notepad app used to manually log daily trip details (right)."}
...
knitr::kable(head(df_display_1, 5), booktabs = TRUE)
```

```{r display-table-2, echo=FALSE}
df_display_2 <- bus_data[ , 6:10]
knitr::kable(head(df_display_2, 5), booktabs = TRUE)
```
\end{center}
...
- **Interpretation:** The data indicates that rain significantly impacts commute comfort. The odds ratio suggests that a crowded bus is approximately **`r round(fisher_result$estimate, 1)` times** more likely to occur on a rainy day than on a clear day.
...
legend("topleft", legend = c("Posterior (rain)", "Prior (general)"),
       col = c("salmon", "lightblue"), lwd = 3, lty = c(1, 2), bty = "n")
```

# Conclusion

This project set out to analyse the reliability and comfort of my school commute using a data-driven approach. By applying a statistical tests to personal bus logs, we have moved beyond anecdotal evidence to identify distinct patterns in travel efficiency and bus overcrowding.
...
| **Can we accurately predict arrival time based on departure?** | Linear Regression | **Yes.** A strong linear relationship exists ($R^2 \approx 0.996$), but random traffic noise creates a residual error of $\pm 15$ minutes regardless of departure time. |
| **Does rain lead to overcrowding?** | Fisher's Exact Test | **Yes.** Rain is a significant predictor of crowding. The odds of a full bus increase dramatically during adverse weather. |
...
| **How much should I worry about rain?** | Bayes's Theorem | **Double the Risk.** The presence of rain acts as a risk multiplier ($1.92\times$), shifting the probability of a crowded bus from a baseline ($0.4$) to a near-certainty ($0.77$). |
\endgroup
...
- **Binary Definition of "Crowded":** The variable `crowded_unilink_to_home` is subjective (`TRUE`/`FALSE`). A more granular metric (e.g., "Seats Available") would allow for more detailed analysis.
