...
The data has `r ncol(bus_data)` columns and `r nrow(bus_data)` rows. A sample of the dataset is displayed below in two parts: the *"To Campus"* details and the *"To Home"* details.
...
- **Interpretation:** The data indicates that rain impacts commute comfort. The odds ratio suggests that the odds of encountering a crowded bus are approximately **`r round(fisher_result$estimate)` times** higher on a rainy day compared to a clear day.
...
| **Does rain lead to overcrowding?** | Fisher's Exact Test | **Yes.** Rain is a predictor of crowding. The odds of a full bus increase dramatically (`r round(fisher_result$estimate)`$\times$) during rain. |
